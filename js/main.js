...
function getWishList(){ //fetch Wish List JSON
	...
	var jqxhr = $.getJSON( 'http://localhost:8888/giftaway/wishList.json', function(wishlist) {
...
function getInfo(){ //fetch Occasion Info JSON 
	...
	var jqxhr = $.getJSON( 'http://localhost:8888/giftaway/occasionInfo.json', function(info) {
	  ...
	  //console.log(info.title);
	  	if( !$('body').hasClass("populated") ){
			...
			for(var i=0, l=info.contacts.length; i<l; i++){
				var contactImg = '';
				if(info.contacts[i].imageUrl==null || info.contacts[i].imageUrl==''){contactImg='img/user.svg'}else{contactImg=info.contacts[i].imageUrl}
